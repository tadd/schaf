...
(describe "=" (lambda ()
  ...
  (noexpect = 0 0 0 0 42)
  (noexpect = 42.0 0)
  (noexpect = 0 0.0 0 0 42.0)))

(describe "<" (lambda ()
  (expect < 2 4)
  (expect < 2 3 4 5)
  (expect < 2.0 4)
  (expect < 2 3.1 4.2 5.9)
  (noexpect < 2 0)
  (noexpect < 2 3 4 4)
  (noexpect < 2.0 0)
  (noexpect < 2 3.1 4.2 4.2)))
...
(describe "max" (lambda ()
  ...
  (expect = (max -0.1 0) 0)
  (expect = (max -0.1 2 10.1) 10.1)
  (expect inexact? (max 1.0 2))))
...
(describe "min" (lambda ()
  ...
  (expect = (min 2.0 2 2) 2)
  (expect = (min -1.0) -1.0)
  (expect = (min -1 -2.1 -3.1) -3.1)
  (expect inexact? (min 1.0 0))))

(describe "+" (lambda ()
  (expect = (+ 42 21) 63)
  (expect integer? (+ 42 21))
  (expect = (+ 42 2.1) 44.1)
  (expect = (+ -1.2 2) 0.8)
  (expect real? (+ -1.2 2))))
...
(describe "special real values" (lambda ()
  (define (nan? x)
    (not (= x x)))
  (let* ((pinf (/ 1 0.0))
         (ninf (/ -1 0.0))
...
(describe "atan" (lambda ()
  ...
  (expect = (atan 0/7) 0.0)
  (expect = (atan 0/8 0/9) 0.0)))

(describe "sqrt" (lambda ()
  (expect = (sqrt 0.0) 0.0)
  (expect = (sqrt 1.0) 1.0)
  (expect = (sqrt 4.0) 2.0)
  (expect = (sqrt 16/4) 2.0)))
...
(describe "inexact->exact" (lambda ()
  ...
  (expect = (inexact->exact -1.1) -1)
  (expect = (inexact->exact -1/2) -1/2)
  (expect exact? (inexact->exact 1))
  (expect exact? (inexact->exact 1.1))
  (expect exact? (inexact->exact -1/2))))
...
(describe "number->string with radix" (lambda ()
  ...
  (expect string=? (number->string 1 10) "1")
  (expect string=? (number->string -1 10) "-1")
  (expect string=? (number->string 16777216 10) "16777216")
  (expect string=? (number->string -16777216 10) "-16777216")

  (expect string=? (number->string 0 2) "0")
  (expect string=? (number->string 1 2) "1")
  (expect string=? (number->string -1 2) "-1")
  (expect string=? (number->string 16777216 2) "1000000000000000000000000")
  (expect string=? (number->string -16777216 2) "-1000000000000000000000000")

  (expect string=? (number->string 0 8) "0")
  (expect string=? (number->string 1 8) "1")
  (expect string=? (number->string -1 8) "-1")
  (expect string=? (number->string 16777216 8) "100000000")
  (expect string=? (number->string -16777216 8) "-100000000")

  (expect string=? (number->string 0 16) "0")
...
(describe "string->number with radix" (lambda ()
  (expect = (string->number "0" 10) 0)
  (expect = (string->number "1" 10) 1)
  (expect = (string->number "-1" 10) -1)
  ...
  (expect = (string->number "-1" 2) -1)
  (expect = (string->number "1000000000000000000000000" 2) 16777216)
  (expect = (string->number "-1000000000000000000000000" 2) -16777216)

  (expect = (string->number "0" 8) 0)
  (expect = (string->number "1" 8) 1)
  (expect = (string->number "-1" 8) -1)
  (expect = (string->number "100000000" 8) 16777216)
  (expect = (string->number "-100000000" 8) -16777216)
...
(describe "string-ci=?" (lambda ()
  (expect string-ci=? "foo" "foo")
  (expect string-ci=? "FOO" "foo")
  (expect string-ci=? "foo" "FOO")
  (expect string-ci=? "Foo" "fOO")
  (expect string-ci=? "A\"B" "a\"b")
  (expect string-ci=? "A\"b" "a\"B")))
...
(describe "string>?" (lambda ()
  ...
  (expect string>? "b" "a")
  (expect string>? "ab" "a")
  (expect string>? "def" "abc")))

(describe "string<=?" (lambda ()
  (expect string<=? "a" "a")
  (expect string<=? "def" "def")
  (expect string<=? "A" "a")
  (expect string<=? "a" "b")
  (expect string<=? "a" "ab")
  (expect string<=? "abc" "def")))
...
(describe "string-ci<?" (lambda ()
  (expect string-ci<? "a" "B")
  (expect string-ci<? "a" "b")
  (expect string-ci<? "a" "ab")
  (expect string-ci<? "abc" "def")))

(describe "string-ci>?" (lambda ()
  (expect string-ci>? "b" "A")
  (expect string-ci>? "b" "a")
  (expect string-ci>? "ab" "a")
  (expect string-ci>? "def" "abc")))

(describe "string-ci<=?" (lambda ()
  (expect string-ci<=? "a" "A")
  (expect string-ci<=? "def" "DEF")
  (expect string-ci<=? "a" "B")
  (expect string-ci<=? "a" "b")
  (expect string-ci<=? "a" "ab")
  (expect string-ci<=? "abc" "def")))
...
(describe "list->string" (lambda ()
  (expect string=? (list->string '(#\a #\b #\c)) "abc")
  (expect string=? (list->string '()) "")
  (expect string=? (list->string '(#\space #\space)) "  ")))
