...
static bool is_pointer(Value v)
{
...
{
    return is_pointer(v) && in_heap_range(v) && is_valid_header(v);
}
...
static void free_val(Value v)
{
    ...
    case TAG_PORT: {
        Port *p = PORT(v);
        if (is_user_opened_file(p->fp))//XXX
            fclose(p->fp);
        p->fp = NULL;//XXX
        return;
    }
...
void gc_fin(void)
{
    sweep(); // no marking, all values are freed
    for (size_t i = 0; i < heap.size; i++) {
