...
static void free_chunk(GCHeader *curr)
{
    ...
    free_val(val);
    if (adjoining_p(curr)) {
        heap->free_list->size += HSIZE(curr->size);
#ifdef DEBUG
...
{
    MSHeap *heap = gc_data;
    for (GCHeader *prev = NULL, *curr = heap->free_list; curr != NULL; prev = curr, curr = curr->next) {
...
// dummy
static void eps_add_root(UNUSED const Value *p) {}
