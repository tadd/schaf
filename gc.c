...
static bool adjoining_p(const Chunk *prev, const Chunk *curr)
{
    ...
    if (prevnext != curr) {
        debug("not adjoining: %p, %p", prevnext, curr);
        return false;
    }
    return true;
}
...
static void sweep(void)
{
    ...
    for (size_t i = 0; i < heaps_length; i++) {
        ...
        for (Chunk *ch, *prev = NULL; p < endp; p += offset, prev = ch) {
            ...
            ch = free_chunk(prev, ch, offset);
            if (prev == ch)
                offset = sizeof(Chunk) + prev->size;
        }
    }
    assert_heaps();
}
...
static void increase_heaps(void)
{
    ...
    uint8_t *beg = (uint8_t *) last, *end = beg + last->size;
    if (heaps_low > beg)
        heaps_low = beg;
    if (heaps_high < end)
        heaps_high = end;
