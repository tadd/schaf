...
static void mark_val(Value v)
{
    ...
    case TAG_PARSER: {
        ...
        TokenType tt = p->prev_token.type;
        if (tt == TOK_TYPE_IDENT || tt == TOK_TYPE_STRING)
            sch_gc_mark(p->prev_token.value);
        return;
    }
    case TAG_ERROR: {
        Error *p = ERROR(v);
...
static void free_val(Value v)
{
    ...
    case TAG_STRING:
        free(OBJ(v)->string);
        OBJ(v)->string = NULL; // XXX
        return;
    case TAG_ENV:
        table_free(ENV(v)->table);
        ENV(v)->table = NULL; // XXX
        ENV(v)->parent = Qfalse; // XXX
        return;
    case TAG_CLOSURE:
    case TAG_PAIR:
    case TAG_CFUNC:
...
static Header *gc_malloc(size_t size)
{
    if (stress)
        gc();
...
SchObject *obj_new(ValueTag t)
{
    ...
    SchObject *p = (SchObject *) gc_malloc(sizeof(SchObject));
    VALUE_TAG(p) = t;
    return p;
}
