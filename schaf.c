...
static void print_object(FILE* f, Value v, Value record, ObjectPrinter printer);

static void print_pair(FILE *f, Value l, Value record, ObjectPrinter printer)
{
    fprintf(f, "(");
...
static void print_vector(FILE *f, Value val, Value record, ObjectPrinter printer)
{
    if (circular_check(f, val, record))
        goto end;
    fprintf(f, "#(");
