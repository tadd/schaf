...
// Need to synchronize order in sch_init!
enum {
    SYM_IF,
    SYM_COND,
    SYM_CASE,
...
static Value eval(Value env, Value args)
{
    ...
    for (;;) {
    next:
        if (sch_value_is_symbol(args))
            return lookup_or_error(env, args);
        if (args == Qnil || !sch_value_is_pair(args))
            return args;
        Value a = car(args);
        Symbol sym;
    ...
    l_let: {
            ...
            env = letenv;
            if (named) {
                Value proc = closure_new(env, cdr(params), body);
...
void sch_init(const uintptr_t *sp)
{
    ...
    // Need to synchronize order with enum { SYM_ ...}!
    intern("if");
    intern("cond");
    intern("case");
    intern("and");
