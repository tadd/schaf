...
static Value env_inherit(Value orig)
{
    Env *e = obj_new(sizeof(Env), TAG_ENV);
    e->body = table_inherit(ENV(orig)->body);
    return (Value) e;
}

static Value env_put(Value env, Value key, Value value)
{
    table_put(ENV(env)->body, key, value);
    return env;
}

static bool env_set(Value env, Value key, Value value)
{
    return table_set(ENV(env)->body, key, value);
}
...
static Value lookup(const Value env, Value name)
{
    Value found = (Value) table_get(ENV(env)->body, name);
...
static Value syn_do(Value env, Value args)
{
    ...
    expect_list_head("do", bindings);
    expect_list_head("do", tests);
    Value doenv = env_inherit(env);
    Value steps = Qnil;
    for (; bindings != Qnil; bindings = cdr(bindings)) {
        Value b = car(bindings);
        expect_type("do", TYPE_PAIR, b);
...
static Value qq(Value env, Value datum, int64_t depth)
{
    ...
    if (a == SYM_QUASIQUOTE) {
        expect_type("nested quasiquote", TYPE_PAIR, d);
...
static Value qq_list(Value env, Value datum, int64_t depth)
{
    ...
    for (Value last = ret; datum != Qnil; datum = cdr(datum)) {
        ...
        if (is_simple || is_quoted_terminal(datum)) {
            expect_type("quasiquote", TYPE_PAIR, cdr(ret));
...
static Value list_tail(const char *func, Value list, Value k)
{
    ...
    if (i != n)
        runtime_error("%s: list is shorter than %"PRId64" element", func, n);
    return p;
}
...
static Value build_apply_args(Value args)
{
    ...
    Value rest = car(args);
    expect_list_head("args on apply", rest);
