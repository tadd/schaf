...
static bool circular_check(FILE *f, Value p, Value record)
{
...
typedef void (*ObjectPrinter)(FILE* f, Value v);
static void visit_object(FILE* f, Value v, Value record, ObjectPrinter printer);

static void visit_pair(FILE *f, Value l, Value record, ObjectPrinter printer)
{
    fprintf(f, "(");
    for (Value p = l, next; p != Qnil; p = next) {
        if (circular_check(f, p, record))
            break;
        record = cons(p, record);
        Value val = car(p);
        if (!circular_check(f, val, record))
            visit_object(f, val, record, printer);
        next = cdr(p);
        if (next == Qnil)
            break;
        fprintf(f, " ");
        if (!value_is_pair(next)) {
            fprintf(f, ". ");
            visit_object(f, next, record, printer);
            break;
        }
    }
    fprintf(f, ")");
...
static void visit_vector(FILE *f, Value val, Value record, ObjectPrinter printer)
{
    if (circular_check(f, val, record))
        goto end;
    fprintf(f, "#(");
    ...
    for (int64_t i = 0, len = scary_length(v); i < len; i++) {
        Value e = v[i];
        if (!circular_check(f, e, record))
            visit_object(f, e, record, printer);
        if (i + 1 == len)
            break;
        fprintf(f, " ");
    }
...
static void visit_object(FILE* f, Value v, Value record, ObjectPrinter printer)
{
    ...
    case TYPE_PAIR:
        visit_pair(f, v, record, printer);
        return;
    case TYPE_VECTOR:
        visit_vector(f, v, record, printer);
        return;
    case TYPE_SYMBOL:
    case TYPE_STRING:
    case TYPE_NULL:
...
{
    visit_object(f, v, Qnil, fdisplay_single);
}
...
typedef void (*ValuePrinter)(FILE *fp, Value v);
...
{
    visit_object(f, v, Qnil, inspect_single);
}
