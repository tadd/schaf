...
{
    Value literals = car(spec), rules = cdr(spec);
    return src;
}
...
static Value expect_transformer_spec(Value spec)
{
    EXPECT(type, TYPE_PAIR, spec);
    Value sym = car(spec);
    if (length(spec) < 2 || sym != SYM_SYNTAX_RULES)
        return runtime_error("malformed transform spec: %s", sch_stringify(spec));
    Value literals = cadr(spec), rules = cddr(spec);
...
static Value syn_let_syntax(Value env, Value args)
{
    ...
    for (Value p = bindings; p != Qnil; p = cdr(p)) {
        ...
        if (length(b) != 2)
            return runtime_error("malformed binding: %s", sch_stringify(b));
        Value ident = car(b), trspec = cadr(b);
        EXPECT(type, TYPE_SYMBOL, ident);
        EXPECT(transformer_spec, trspec);
...
#define DEF_SYMBOL(var, name) SYM_##var = sch_symbol_new(name)
    ...
    // 4.3. Macros
    // 4.3.1. Binding constructs for syntactic keywords
    define_syntax(esyn, "let-syntax", syn_let_syntax, -1); // with syntax-rules
