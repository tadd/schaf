...
static Value apply_typed_cfunc_1_int(Value env, Value f, Value args)
{
    Value a = car(args);
    EXPECT(type, TYPE_INT, a);
    return CFUNC(f)->f1_int(env, value_to_int(a));
}

static Value apply_typed_cfunc_1_str(Value env, Value f, Value args)
{
    Value a = car(args);
    EXPECT(type, TYPE_STRING, a);
    return CFUNC(f)->f1_str(env, value_to_string(a));
}
...
static Value cfunc_new(ValueTag tag, const char *name, void *cfunc, int64_t arity, int8_t *types)
{
    ...
    case 1:
        if (typed) {
            if (types[0] == TYPE_INT)
                f->proc.apply = apply_typed_cfunc_1_int;
            else if (types[0] == TYPE_STRING)
                f->proc.apply = apply_typed_cfunc_1_str;
            else
                f->proc.apply = apply_typed_cfunc_1;
        } else
...
{
    return cfunc_new(TAG_CFUNC, name, cfunc, arity, NULL);
}

static Value typed_cfunc_new(ValueTag tag, const char *name, void *cfunc, int64_t arity, int8_t types[])
{
    Value func = cfunc_new(tag, name, cfunc, arity, types);
    CFunc *f = CFUNC(func);
    if (arity > 0)
...
{
    return cfunc_new(TAG_SYNTAX, name, cfunc, arity, NULL);
}
...
static void define_syntax_n(Value env, const char *name, void *cfunc, int64_t arity)
{
    env_put(env, value_of_symbol(name), value_of_syntax(name, cfunc, arity));
}

static void define_procedure_n(Value env, const char *name, void *cfunc, int64_t arity)
{
...
static void define_procedure_n_typed(Value env, const char *name, void *cfunc,
                                   int64_t arity, ...)
{
    expect_cfunc_arity(arity);
...
#if 0
static void define_syntax_n_typed(Value env, const char *name, void *cfunc,
                                int64_t arity, ...)
{
    expect_cfunc_arity(arity);
    int8_t types[CFUNCARG_MAX];
...
static Value proc_abs(UNUSED Value env, int64_t x)
{
    return value_of_int(x < 0 ? -x : x);
}
...
static Value proc_load(UNUSED Value env, const char *path)
{
...
#define ARITY(f) _Generic((f), \
    ...
    Value (*)(Value, const char *): 1, \
    Value (*)(Value, Value, Value): 2, \
    Value (*)(Value, Value, Value, Value): 3, \
    default: -1)
#define DEFINE_XANY(def, e, name, f, cond, n) do { \
        static_assert(cond, "invalid type function"); \
        def(e, name, f, n); \
    } while (0)
#define DEFINE_ANY(def, e, name, f) DEFINE_XANY(def, e, name, f, ARITY(f) >= 0 && ARITY(f) <= 3, ARITY(f))
#define DEFINE_ANY_V(def, e, name, f) DEFINE_XANY(def, e, name, f, ARITY(f) == 1, -1)
...
#define ARG_TYPE_1(f) _Generic((f), \
    Value (*)(Value, Value): TYPE_ANY, \
    Value (*)(Value, int64_t): TYPE_INT, \
    Value (*)(Value, const char *): TYPE_STRING, \
    default: TYPE_UNDEF)

#define define_procedure_typed(e, name, f) do { \
        static_assert(ARITY(f) >= 0 && ARITY(f) <= 3, "invalid type function"); \
        static_assert(ARG_TYPE_1(f) != TYPE_UNDEF, "invalid type function"); \
        define_procedure_n_typed(e, name, f, ARITY(f), ARG_TYPE_1(f)); \
    } while (0)
...
#define DEF_SYMBOL(var, name) SYM_##var = value_of_symbol(name)
    ...
    define_procedure_typed(e, "abs", proc_abs);
    ...
    // 6.2.6. Numerical input and output
    define_procedure(e, "number->string", proc_number_to_string);
    // 6.3. Other data types
    // 6.3.1. Booleans
...
#define DEFUN_CXR(x, y) define_procedure(e, "c" #x #y "r", proc_c##x##y##r);
    ...
    define_procedure(e, "set-car!", proc_set_car);
    define_procedure(e, "set-cdr!", proc_set_cdr);
    ...
    define_procedure_v(e, "values", proc_values);
    define_procedure(e, "call-with-values", proc_call_with_values);
    //- dynamic-wind
    // 6.5. Eval
    define_procedure(e, "eval", proc_eval);
    define_procedure(e, "scheme-report-environment", proc_scheme_report_environment);
    define_procedure(e, "null-environment", proc_null_environment);
    define_procedure(e, "interaction-environment", proc_interaction_environment);
    // 6.6. Input and output
    // 6.6.1. Ports
    define_procedure(e, "port?", proc_port_p); // R5RS missing the definition!!
    define_procedure(e, "current-input-port", proc_current_input_port);
    define_procedure(e, "current-output-port", proc_current_output_port);
    define_procedure(e, "open-input-file", proc_open_input_file);
    define_procedure(e, "close-port", proc_close_port);
    define_procedure(e, "close-output-port", proc_close_port); // alias
    define_procedure(e, "close-input-port", proc_close_port);  // alias
    // 6.6.2. Input
    define_procedure_v(e, "read", proc_read);
    // 6.6.3. Output
    define_procedure(e, "display", proc_display);
    define_procedure(e, "newline", proc_newline);
    // 6.6.4. System interface
    define_procedure_typed(e, "load", proc_load);
    ...
    define_procedure(e, "schaf-environment", proc_schaf_environment);
