...
static Value syn_letrec(Value env, Value args)
{
    ...
    for (Value p = bindings; p != Qnil; p = cdr(p)) {
        Value b = car(p);
        Pair *p = get(PAIR, b);
        Symbol ident = get(SYMBOL, p->car);
        Value val = eval(letenv, car(p->cdr));
        CHECK_ERROR(val);
        env_put(letenv, ident, val);
    }
    return eval_body(letenv, body);
...
static Value dup_list(Value l, Value *plast)
{
    ...
    for (Value p = l; p != Qnil; p = cdr(p))
        last = PAIR(last)->cdr = list1(get(PAIR, p)->car);
    *plast = last;
    return cdr(dup);
}
...
{
    size_t len = scary_length(get(VECTOR, v));
    return sch_integer_new(len);
}

static Value proc_vector_ref(UNUSED Value env, Value o, Value k)
{
    Value *v = get(VECTOR, o);
