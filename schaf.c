...
#define DEFINE_ANY(def, e, name, f) DEFINE_XANY(def, e, name, f, ARITY(f) >= 0, ARITY(f))
...
#define DEF_SYMBOL(var, name) SYM_##var = sch_symbol_new(name)
    ...
    // 4.2.5. Delayed evaluation
    define_syntax(e, "delay", syn_delay);
    // 4.2.6. Quasiquotation
...
#define DEFUN_CXR(x, y) define_procedure(e, "c" #x #y "r", proc_c##x##y##r);
    ...
    define_procedure(e, "assoc", proc_assoc);
    // 6.3.3. Symbols
    ...
    define_procedure(e, "substring", proc_substring);
    define_procedure_v(e, "string-append", proc_string_append);
    // 6.3.6. Vectors
    define_procedure(e, "vector?", proc_vector_p);
    define_procedure_v(e, "make-vector", proc_make_vector);
    define_procedure_v(e, "vector", proc_vector);
    define_procedure(e, "vector-length", proc_vector_length);
    define_procedure(e, "vector-ref", proc_vector_ref);
    define_procedure(e, "vector-set!", proc_vector_set);
    // 6.4. Control features
    ...
    define_procedure(e, "force", proc_force);
    ...
    define_procedure(e, "dynamic-wind", proc_dynamic_wind);
    // 6.5. Eval
    ...
    // 6.6. Input and output
    // 6.6.1. Ports
    define_procedure(e, "call-with-input-file", proc_call_with_input_file);
    define_procedure(e, "call-with-output-file", proc_call_with_output_file);
    define_procedure(e, "port?", proc_port_p); // R5RS missing the definition!!
    define_procedure(e, "input-port?", proc_input_port_p);
    define_procedure(e, "output-port?", proc_output_port_p);
    ...
    define_procedure(e, "with-input-from-file", proc_with_input_from_file);
    define_procedure(e, "with-output-to-file", proc_with_output_to_file);
    define_procedure(e, "open-input-file", proc_open_input_file);
    define_procedure(e, "open-output-file", proc_open_output_file);
    ...
    define_procedure(e, "eof-object?", proc_eof_object_p);
    // 6.6.3. Output
    define_procedure_v(e, "display", proc_display);
    define_procedure_v(e, "newline", proc_newline);
    // 6.6.4. System interface
    ...
    define_procedure_v(e, "read-string", proc_read_string);
    define_procedure(e, "open-output-string", proc_open_output_string);
    define_procedure(e, "get-output-string", proc_get_output_string);
    // (scheme process-context)
    define_procedure_v(e, "exit", proc_exit);
    // (scheme lazy)
    define_procedure(e, "promise?", proc_promise_p);

    // Local Extensions
    define_syntax(e, "_defined?", syn_defined_p);
    define_procedure(e, "_cputime", proc_cputime);
    define_procedure_v(e, "p", proc_p);
