...
static Value proc_eqv(UNUSED Value env, Value x, Value y)
{
    ...
    if (!sch_value_is_bignum(x) || !sch_value_is_bignum(y))
        return Qfalse;
    return OF_BOOL(bigint_eq(BIGNUM(x), BIGNUM(y)));
}
...
Value bignum_normalize(BigInt *x)
{
    if (BIGINT_FIXNUM_MAX == NULL) { // init
        BIGINT_FIXNUM_MAX = bigint_from_int(INT64_MAX >> 1U);
        BIGINT_FIXNUM_MIN = bigint_from_int(-((INT64_MAX >> 1U) + 1));
    }
...
static BigInt *ensure_bigint(Value x)
{
    if (sch_value_is_bignum(x))
        return BIGNUM(x);
    return bigint_from_int(sch_integer_to_cint(x));
}
...
static Value int_mul(Value x, Value y)
{
    bool fx = sch_value_is_fixnum(x), fy = sch_value_is_fixnum(y);
    if (fx && fy) {
        int64_t ix = sch_integer_to_cint(x), iy = sch_integer_to_cint(y);
...
static Value int_div(Value x, Value y)
{
    bool fx = sch_value_is_fixnum(x), fy = sch_value_is_fixnum(y);
    if (fy) {
        int64_t iy = sch_integer_to_cint(y);
        if (iy == 0)
            return Qfalse; // divzero!
        if (fx)
            return fixnum_normalize(sch_integer_to_cint(x) / iy);
    }
    if (fx && !fy)
...
static Value proc_positive_p(UNUSED Value env, Value obj)
{
    if (!sch_value_is_integer(obj))
        return Qfalse;
    bool b = sch_value_is_fixnum(obj) ?
        sch_fixnum_to_cint(obj) > 0 : bigint_is_positive(BIGNUM(obj));
    return OF_BOOL(b);
}
...
static Value proc_negative_p(UNUSED Value env, Value obj)
{
    if (!sch_value_is_integer(obj))
        return Qfalse;
    bool b = sch_value_is_fixnum(obj) ?
        sch_fixnum_to_cint(obj) < 0 : bigint_is_negative(BIGNUM(obj));
    return OF_BOOL(b);
}
...
static Value proc_odd_p(UNUSED Value env, Value obj)
{
    if (!sch_value_is_integer(obj))
        return Qfalse;
    bool b = sch_value_is_fixnum(obj) ?
        (sch_fixnum_to_cint(obj) % 2) != 0 : bigint_is_odd(BIGNUM(obj));
    return OF_BOOL(b);
}
...
static Value proc_even_p(UNUSED Value env, Value obj)
{
    if (!sch_value_is_integer(obj))
        return Qfalse;
    bool b = sch_value_is_fixnum(obj) ?
        (sch_fixnum_to_cint(obj) % 2) == 0 : bigint_is_even(BIGNUM(obj));
    return OF_BOOL(b);
}
...
static Value proc_sub(UNUSED Value env, Value args)
{
    ...
    Value p = cdr(args);
    if (p == Qnil)
        return sch_fixnum_new(-sch_fixnum_to_cint(y));
    for (; p != Qnil; p = cdr(p))
...
static Value proc_div(UNUSED Value env, Value args)
{
    ...
    Value p = cdr(args);
    if (p == Qnil)
        return int_div(sch_fixnum_new(1), y);
    for (; p != Qnil; p = cdr(p)) {
        Value x = get_int(car(p));
        if (sch_value_is_fixnum(x) && sch_integer_to_cint(x) == 0)
...
{
    int64_t a = get_cint(x), b = get_non_negative_int(y);
    int64_t c;
    if (b == 0)
        c = 1;
    else if (a == 0)
...
static Value proc_list_ref(UNUSED Value env, Value list, Value vk)
{
    ...
    if (tail == Qnil)
        return runtime_error("list is not longer than %"PRId64, sch_fixnum_to_cint(k));
    return car(tail);
}
...
{
    if (!sch_value_is_fixnum(version) || sch_fixnum_to_cint(version) != 5)
...
static Value proc_exit(UNUSED Value env, Value args)
{
    ...
    if (args != Qnil) {
        ...
        else if (sch_value_is_fixnum(obj))
            exit_status = sch_fixnum_to_cint(obj);
        else // or #f too
