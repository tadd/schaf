...
#if 0
static Value env_id(Value literals)
{
    Value e = env_new("id table for macro");
    for (Value p = literals; p != Qnil; p = cdr(p)) {
        Value sym = car(p);
        env_put(e, sym, sym);
    }
    return e;
...
static Value located_cons(Value a, Value d)
{
...
static Value copy_pos(Value dst, Value loc)
{
    LOCATED_PAIR(dst)->pos = LOCATED_PAIR(loc)->pos;
    return dst;
}

static Value located_cons_copy_pos(Value a, Value d, Value loc)
{
    Value p = located_cons(a, d);
    return copy_pos(p, loc);
}

static Value located_list1_copy_pos(Value v, Value loc)
{
    return located_cons_copy_pos(v, Qnil, loc);
}
...
static Value transform_apply(Value env, Value l)
{
    ...
    if (!sch_value_is_procedure(syn))
        return located_cons_copy_pos(syn, trargs, l);
...
static Value syntax_rule_parse_params(Value params, int64_t *arity, int64_t *arity_min)
{
    int64_t len = 0;
    Value p = params, ret = DUMMY_PAIR(), last = ret;
    ...
    if (p == SYM_DOTS) {
        if (len == 0)
            return runtime_error("rule pattern must have one named argument at least");
        *arity = -1;
        *arity_min = len - 1;
        return cdr(ret);
    }
...
static Value match_literals(Value lpos, Value params UNUSED, Value args UNUSED)
{
    int64_t prev_pos = 0;
    for (Value p = lpos; p != Qnil; p = cdr(p)) {
        (void) prev_pos;//...
    }
    return Qfalse;
}
...
static Value append_dots(Value l)
{
    Value p, dup = dup_list(l, &p);
    if (dup != Qnil)
        PAIR(p)->cdr = list1(SYM_DOTS);
    return dup;
}

#define EXPECT_MATCH_LITERALS(lposs, params, args) EXPECT_ERROR(match_literals(lpos, params, args))
...
static Value matcher_arity_min(Value data, Value args)
{
    ...
    int64_t min = sch_integer_to_cint(arity_min);
    EXPECT_ARITY_MIN(min, args);
    return body; // FIXME!
}
...
static Value matcher_literals(Value data, Value args)
{
    ...
    int64_t a = sch_integer_to_cint(arity);
    EXPECT_ARITY(a, args);
    EXPECT_MATCH_LITERALS(lpos, params, args);
    return body; // FIXME!
}
...
static Value matcher_literals_and_arity_min(Value data, Value args)
{
    ...
    Value params = caddr(data), body = cdddr(data);
    int64_t min = sch_integer_to_cint(arity_min);
    EXPECT_ARITY_MIN(min, args);
    EXPECT_MATCH_LITERALS(lpos, params, args);
    return body; // FIXME!
                                           ...
                                           Value params, Value body)
{
    ...
    if (lpos == Qnil) {
        ...
        Value dparams = append_dots(params);
        data = cons(closure_new(env_macro, dparams, body), data);
        data = cons(dparams, data);
        data = cons(sch_integer_new(arity_min), data);
        return cfunc_closure_new(__FUNCTION__, matcher_arity_min, -1, data);
    }
    if (arity_min == 0) {
        data = cons(closure_new(env_macro, params, body), data);
        data = cons(sch_integer_new(arity), data);
        data = cons(lpos, data);
        return cfunc_closure_new(__FUNCTION__, matcher_literals, -1, data);
    }
    Value dparams = append_dots(params);
    data = cons(closure_new(env_macro, dparams, body), data);
    data = cons(dparams, data);
    data = cons(sch_integer_new(arity_min), data);
    data = cons(lpos, data);
    return cfunc_closure_new(__FUNCTION__, matcher_literals_and_arity_min, -1, data);
}
                                  ...
                                  Value params, Value body)
{
    ...
    if (arity == 0 || literals == Qnil)
        return closure_new(env_macro, params, body);
    return matching_closure_with_precond(literals, arity, 0, params, body);
}
...
static Value syntax_rule_new(Value literals, Value params, Value template)
{
    ...
    int64_t arity = 0, arity_min = -1;
    Value clparams = syntax_rule_parse_params(params, &arity, &arity_min);
    EXPECT_ERROR(clparams);
    r->arity_min = arity_min;
    Value c = matching_closure_new(literals, arity, arity_min, clparams, template);
...
static Value transformer_new(Value literals, Value rules)
{
    ...
    for (Value p = rules; p != Qnil; p = cdr(p)) {
        ...
        scary_push((void ***) &tr_rules, (void *) ruleval);
    }
    return (Value) tr;
}
