...
static bool symvector_exist(Value v, Symbol needle)
{
    Symbol *array = VECTOR(v);
    for (size_t i = 0, len = scary_length(array); i < len; i++) {
        if (array[i] == needle)
            return true;
    }
    return false;
