...
static Value toplevel_environment = Qnil; // list of frames
...
{
    Value frame = car(env);
    frame = cons(cons(name, val), frame);
    PAIR(env)->car = frame;
}
...
#if 0
static inline Value dup_pair(Value p)
{
    return cons(car(p), cdr(p));
}

static Value append2_dup(Value l1, Value l2)
{
    if (l1 == Qnil)
        return dup_pair(l2);
    if (l2 == Qnil)
        return dup_pair(l1);
    return append2(l1, l2);
}
#endif
...
//PTR
static Value apply_closure(ATTR(unused) Value env, Value proc, Value args)
{
    Closure *cl = CLOSURE(proc);
    int64_t arity = cl->proc.arity;
    Value clenv = cons(Qnil, cl->env), params = cl->params;
    if (arity == -1)
...
{
    for (; env != Qnil; env = cdr(env)) {
        Value frame = car(env);
        Value found = assq(name, frame);
        if (found != Qfalse)
            return found;
    }
    return Qfalse;
...
static Value syn_do(Value env, Value args)
{
    ...
    Value doenv = cons(Qnil, env), steps = Qnil;
    for (Value p = bindings; p != Qnil; p = cdr(p)) {
        Value b = car(p);
        expect_nonnull("do", b);
...
static void initialize(void)
{
    ...
    toplevel_environment = list1(Qnil);
