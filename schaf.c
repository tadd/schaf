...
static Value proc_positive_p(UNUSED Value env, Value obj)
{
    if (!sch_value_is_integer(obj))
        return Qfalse;
    return OF_BOOL(int_is_positive(obj));
}
...
static Value proc_negative_p(UNUSED Value env, Value obj)
{
    if (!sch_value_is_integer(obj))
        return Qfalse;
    return OF_BOOL(int_is_negative(obj));
}
...
{
    if (sch_value_is_integer(version) && sch_fixnum_to_cint(version) == 5)
        return Qfalse;
